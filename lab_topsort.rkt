...
#|-------------------------------------------------------------------------------
 ...
 | Pledge: I pledge my honor that I have abided by the stevenes honor system
 |-------------------------------------------------------------------------------|#
...
;; 5 PTS
(define (indegree E v)
  (let loop ((edges E) (count 0))
    (cond
      ((null? edges) count)
      ((equal? v (cadr (car edges)))
       (loop (cdr edges) (+ count 1)))
      (else (loop (cdr edges) count))))
)
...
;; 5 PTS
(define (find-top G)
  (let ((V (get-vertices G))
        (E (get-edges G)))
    (let loop ((i 0))
      (cond
        ((>= i (length V)) (topsort-error))
        ((zero? (indegree E (list-ref V i))) (list-ref V i))
        (else (loop (+ i 1))))))
)
...
(define (topsort G)
  (let loop ((G G) (res '()))
    (cond
      ((null? (get-vertices G)) (reverse res))
      (else
        (let ((v (find-top G)))
          (loop (pop-vertex G v) (cons v res))))))
)